...
function searchTrip() {
    ...
    if (start_id==0) {
       alert('Vui lĂ²ng chá»n Ä‘iá»ƒm Ä‘i')
       $('#start_id').focus();
    }else if(end_id==0){
       alert('Vui lĂ²ng chá»n Ä‘iá»ƒm Ä‘áº¿n');
       $('#end_id').focus();
    }else if(start_date==''){
       alert('Vui lĂ²ng chá»n ngĂ y Ä‘i');
       $('#start_date').focus();
    }else{
...
function chooseTripRent(node_id){
    var url = baseUrl+'/trip/ChooseSeatRent';
    $.ajax({
        type: "POST",
        url: url,
...
function chooseSeat(obj){
    ...
    $('#total_cart_small').html(NumToStr(total_cart)+'Ä‘');
    $('#total_cart_big').html(NumToStr(total_cart)+' VNÄ');
...
function validate(){
    ...
    $.ajax({
        type: "POST",
        url: url,
        data: ({fullname:fullname,mobile:mobile,code:code}),
        dataType: "json",
...
function Order(status){
    ...
    if (fullname=='') {
        alert('Há» vĂ  tĂªn khĂ´ng nĂªn Ä‘á»ƒ trá»‘ng');
        UIkit.modal('#modal-pttt').hide();
        $('#fullname').focus();
    }else if(mobile==''){
        alert('Di Ä‘á»™ng khĂ´ng nĂªn Ä‘á»ƒ trá»‘ng');
        UIkit.modal('#modal-pttt').hide();
        $('#mobile').focus();
    }else if(pm_id==0){
        alert('Vui lĂ²ng chá»n phÆ°Æ¡ng thá»©c thanh toĂ¡n');
    }else if(pm_id==5 && pm_bank_id==0){
        alert('Vui lĂ²ng chá»n ngĂ¢n hĂ ng');
    }else{
        var url = baseUrl+'/trip/Place';
        $.ajax({
            type: "POST",
            url: url,
            data: ({fullname:fullname,mobile:mobile,code:code,note:note,pm_id:pm_id,pm_bank_id:pm_bank_id,status:status}),
            dataType: "json"
        }).done(function( data ){
           if (data.success==1) {
              window.location.href = data.url;
...
function cancelTicket(){
    ...
    if (n==0) {
        alert('Vui lĂ²ng chá»n chiá»u muá»‘n huá»·');
    }else{
        if ( confirm('Báº¡n cĂ³ cháº¯c cháº¯n muá»‘n huá»· vĂ©?') ) {
...
function editTicket(){
    var n = $( "input:checked" ).length;
    if (n==0) {
        alert('Vui lĂ²ng chá»n chiá»u muá»‘n sá»­a');
    }else{
        if ( confirm('Báº¡n cĂ³ cháº¯c cháº¯n muá»‘n sá»­a vĂ©?') ) {
     ...
     else
     {
         switch(format)
         {
           case 0:
                ...
                //document.all.countdown.innerHTML = Time_Left + ' giĂ¢y';
                break;
           case 1:
                //More datailed.
                ...
                if(days)
                {
                    document.getElementById("countdown").innerHTML= days + ' ngĂ y' + dps + ' ';
                    document.getElementById("countdown").innerHTML += hours + ' giá»' + hps + ' ';
                    document.getElementById("countdown").innerHTML += minutes + ' phĂºt' + mps + ' ';
                    document.getElementById("countdown").innerHTML += seconds + ' giĂ¢y' + sps;
                }   
                else
                {
                    if(hours)
                ...
                break;
           default: 
                document.getElementById("countdown").innerHTML = Time_Left + ' giĂ¢y';
         }
...
function searchCarDay() {
    ...
    if (start_id==0) {
       ...
       alert('Vui lĂ²ng chá»n ngĂ y nháº­n xe');
       $('#start_date').focus();
    }else if(start_hour==''){
       alert('Vui lĂ²ng chá»n giá» nháº­n xe');
       $('#start_hour').focus();
    }else if(end_date==''){
       alert('Vui lĂ²ng chá»n ngĂ y tráº£ xe');
       $('#end_date').focus();
    }else if(end_hour==''){
       alert('Vui lĂ²ng chá»n giá» tráº£ xe');
       $('#end_hour').focus();
    }else{
...
function searchCarTrip() {
    ...
    if (from_address=='') {
       alert('Vui lĂ²ng chá»n Ä‘iá»ƒm Ä‘Ă³n')
       $('#from_address').focus();
    }else if(to_address==''){
       alert('Vui lĂ²ng chá»n Ä‘iá»ƒm Ä‘áº¿n');
       $('#to_address').focus();
...
function Usecode(act){
    var code = $('#promotion_code').val();
    var url = baseUrl+'/member/usecode';
    $.ajax({
        type: "POST",
        url: url,
        data: {code:code},
        dataType: "json"
         ...
         UIkit.modal('#makhuyenmai').show();
        if(data.status==1) {
            //thanh cong
            $('#titelkm').html("ThĂ nh cĂ´ng");
            $('#contentkm').html(data.msg);
            $('#promotion_code').val('');
            if(act==1){
                //goi tu trang order
                window.location.reload().delay(5);
            }else{
                //goi tu trang profile
...
function CloseModalKM(){
     UIkit.modal('#makhuyenmai').hide();
}
function Updatecode(){
    var intrcode = $('#introduction_code').val();
    var url = baseUrl+'/member/updatecode';
    $.ajax({
        type: "POST",
        url: url,
        data: {intrcode:intrcode},
        dataType: "json"
    }).done(function( data ){
           UIkit.modal('#updatecodekm').show();
           $('#msgupdatecode').html(data.msg);
    });
}
function CloseModalUpdateKM(){
     UIkit.modal('#updatecodekm').hide();
}
function Listcode(){
    var url = baseUrl+'/member/listcode';
    $.ajax({
        type: "POST",
        url: url,
        dataType: "json"
    }).done(function( data ){
           UIkit.modal('#taikhoankhuyenmai').show();
           $('#msgtaikhoankhuyenmai').html(data.msg);
    });
}
function CloseModalListKM(){
     UIkit.modal('#taikhoankhuyenmai').hide();
}